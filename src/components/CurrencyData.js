...
const CurrencyData = ({currency,fiat}) => {
    ...
    const fetchData= async () =>{
        const response= await fetch(`${url}/ticker${key}&ids=${currency}&convert=${fiat}`);
        const jsonResponse= await response.json();
        return jsonResponse;
    }

    const fetchMetadata = async () =>{
        const response= await fetch(`${url}${key}&ids=BTC,ETH,ADA,DOT,XRP,SOL,BNB,LINK,DOGE,MATIC,LTC,UNI,LUNA,THETA,ATOM,BAT,USDT,BCH,ICP,ALGO,AVAX,AUDIO&attributes=id,website_url`);
        const jsonResponse= await response.json();
        return jsonResponse;
    }
    ...
    const setData= async ()=>{
        ...
        setRank(()=>{if(data[0]['rank']==='1'){return(`${data[0]['rank']}st`)}
                    else if(data[0]['rank']==='2'){return(`${data[0]['rank']}nd`)}
                    else if(data[0]['rank']==='3'){return(`${data[0]['rank']}rd`)}
                    else{return(`${data[0]['rank']}th`)}
        });

        setPrice(`${Number(data[0]['price']).toFixed(2)} ${fiat}`);
        setName(`${data[0].id}`);
        setCap(`${Number(data[0]['market_cap']).toLocaleString()} ${fiat}`);
        setVolume(`${Number(Number(data[0]['1d']['volume']).toFixed(0)).toLocaleString()} coins`);
        setLimits(()=>{if(data[0].hasOwnProperty('max_supply')){
